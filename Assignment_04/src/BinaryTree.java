...
public class BinaryTree<T> {
	...
	public static <T> boolean heightBalanced(BinaryTree<T> t) {
		...
		else if(Math.abs(height(t.getLeft())-height(t.getRight()))>1) return false;
		else return heightBalanced(t.getRight())&&heightBalanced(t.getLeft());
	}
	...
	public static <T> void levelOrder(BinaryTree<T> t) {
		...
		while (list.isEmpty()==false) {
			BinaryTree<T> curr = list.get(0);
			System.out.print(curr.getData() + " ");

			if (curr.getLeft() != null) list.add(curr.getLeft());
			if (curr.getRight() != null) list.add(curr.getRight());
